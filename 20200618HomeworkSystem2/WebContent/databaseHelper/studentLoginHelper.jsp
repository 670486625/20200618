...
<jsp:useBean id="mario" class="jdbc.Mario" />
	<%
		...
		//只判断一条数据
		if (rs.next()) {
			if (userpassword.equals(rs.getObject("studentPassword"))) {
				//设置session
				...
				out.print("<script>alert('密码错误'); window.location='../jsp/studentLogin.jsp' </script>");
			}
		} else {
			out.print("<script>alert('无此账号'); window.location='../jsp/studentLogin.jsp' </script>");
		}
